// gen/src/api-module-generator.ts
import * as fs from 'fs';
import * as path from 'path';
import { Table, DbReaderConfig } from './interfaces';
import { loadTemplate } from './utils/template-loader';

export class ApiModuleGenerator {
  ...
  generateModules() {
    ...
    console.log(`API Modules have been generated in ${outputDir}`);
  }

  private generateModuleContent(entityName: string, kebabCaseName: string): string {
    return loadTemplate('api-module.template.ejs', {
      entityName,
      kebabCaseName,
    });
  }
