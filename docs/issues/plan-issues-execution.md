...
- **Teste CLI com SQLite (em disco):** `node test/e2e-generator/projects/todo/db/create-sqlite-fixture.js ./output` → sucesso; `node dist/main.js -a cli-test ... -d ./output/fixture.sqlite -o ./output -t sqlite -f "entities,services,interfaces,controllers,dtos,modules,app-module,main,env,package.json,readme,datasource,diagram"` → **sucesso**. Artefatos gerados: `db.reader.sqlite.json`, `src/app/entities/user.ts`, `src/app/user/*`, `src/app/app.module.ts`, `src/app/main.ts`, `src/app/config/datasource.service.ts`, `.env`, `package.json`, `README.md`, `public/diagram.png`. Plano de execução: [plan-cli-test-execution.md](plan-cli-test-execution.md).
...
| 3 | `node test/e2e-generator/projects/todo/db/create-sqlite-fixture.js ./output` | Sucesso |
