...
# Changelog — E2E Postgres todo e validação 12/12
...
- **Problema:** Em todo/postgres o generator conectava em `database=todo`, mas esse banco não era criado no Postgres (apenas `selling` e `schedule` em `init-postgres.js` e em `00-init-extra.sh`), resultando em 0 tabelas e FALHA na aferição (5/9).
- **Solução:** Inclusão de `todo` no init do Postgres: em `init-postgres.js` (executado pelo entrypoint E2E) e em `pg-init/00-init-extra.sh` (init do container com volume novo).
