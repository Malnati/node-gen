...
- **Modificados**
  ...
  - `test/e2e-generator-mock/run.js` — verificação de conectividade dos containers de banco (postgres, mysql, sqlserver) no início do fluxo com timeout de 1,5 s; constante `DB_CONNECT_CHECK_TIMEOUT_MS = 1500`.
...
- Arquivos específicos de projeto permanecem em `test/e2e-generator-mock/projects/<name>/db/`; script de orquestração do Postgres E2E em `.docker/`.
