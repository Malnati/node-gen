...
- `test/e2e-generator-mock/init-sqlserver.js` — criado: script Node que cria o banco `todo` e aplica o schema (idempotente); ajustado para usar `new mssql.ConnectionPool(...).connect()` por banco (master/todo), evitando reaproveitar pool global e garantindo que o schema seja aplicado em `todo`.
