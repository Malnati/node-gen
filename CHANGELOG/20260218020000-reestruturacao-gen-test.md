...
- **`test/`** — testes e mocks:
  ...
  - **`test/e2e-generator/`** — testes e2e do gerador contra o mock (`run.js`, `out/`).
...
- Da raiz para **test/**: `mock/`, `e2e-generator/`, `build-cli-test/`.

### Arquivos criados/alterados

- **Raiz:** novo `package.json` com scripts `build` (delega a `gen`) e `test:e2e` (executa `test/e2e-generator/run.js`).
- **test/e2e-generator/run.js:** paths atualizados: `REPO_ROOT` (dois níveis acima), `GEN_DIR`, `MOCK_DIR` em `test/mock/`, `DIST_MAIN` em `gen/dist/main.js`; `runGenerator` com `cwd: GEN_DIR`.
- **test/mock/README.md**, **test/e2e-generator/README.md:** referências a `test/mock/`, `gen/`, comandos atualizados.
...
- Testes e2e: `npm run test:e2e` (raiz) ou `node test/e2e-generator/run.js`.
