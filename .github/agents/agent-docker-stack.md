---
name: Docker Stack
description: (docker-compose, Dockerfile e entrypoint), garante conformidade da estrutura Docker por aplicativo, centralizando configuração no docker-compose e padronizando variáveis de ambiente com defaults (incluindo sensíveis com defaults fictícios).
version: 1.0.4
referenced_by: opencode.json, AGENTS.md
see_also: AGENTS.md, agent-bash.md, agent-yml-structure.md
---

<!-- .github/agents/agent-docker-stack.md -->

# Agente: Docker Stack

> **📋 Documentação relacionada:**  
> - Índice geral: [AGENTS.md](../../AGENTS.md)  
> - Configuração OpenCode: [opencode.json](../../opencode.json) (seção `agent.docker_stack`)  
...
- **`agent-bash.md`**: Define políticas detalhadas de Bash scripting. Todo script `entrypoint.sh` deve seguir as premissas de Bash definidas naquele agente, incluindo:
...
- **`agent-yml-structure.md`**: Define hierarquia YAML explícita para `docker-compose.yml`.
...
3. Responsabilidade por arquivo:
   ...
   - **entrypoint.sh**:
     ...
     - **Deve seguir todas as políticas de Bash scripting definidas em `agent-bash.md`** (ferramentas GNU, uso de `curl`, evitar HEREDOC, `set -euo pipefail`, etc.).
   - **docker-compose.yml**:
...
1. **Descoberta dos aplicativos/serviços**
   - Identificar, no repositório, os serviços/aplicativos existentes (pelo menos por:
     - Estruturas de pastas (`apps/`, `services/`, `backend/`, `test/e2e-generator/projects/todo/db/` etc.).
