...
if [ "${DB_TYPE}" = "sqlite" ]; then
  sqlite3 "$2" < test/e2e-generator/projects/todo/db/database.sqlite.ddl
  sqlite3 "$2" < test/e2e-generator/projects/todo/db/database.sqlite.sql
elif [ "${DB_TYPE}" = "mysql" ]; then
  mysql -u "$2" -p"$3" -e "CREATE DATABASE IF NOT EXISTS $2;"
  mysql -u "$2" -p"$3" "$2" < test/e2e-generator/projects/todo/db/database.mysql.ddl
  mysql -u "$2" -p"$3" "$2" < test/e2e-generator/projects/todo/db/database.mysql.sql
  ...
  sqlcmd -S "$4" -U "$2" -P "$3" -d "$2" -i test/e2e-generator/projects/todo/db/database.sqlserver.ddl
  sqlcmd -S "$4" -U "$2" -P "$3" -d "$2" -i test/e2e-generator/projects/todo/db/database.sqlserver.sql
fi
