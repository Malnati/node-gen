	...
	public async getSchemaInfo() {
      ...
      `
			...
			for (const tableName of tables) {
				const tableCommentQuery = `SELECT obj_description('"public"."${tableName}"'::regclass) AS table_comment`
				const tableCommentResult = await client.query<{
					table_comment: string | null
				}>(tableCommentQuery)
				const tableComment =
					tableCommentResult.rows[0]?.table_comment || null
				const columnsQuery = `
		SELECT
		  c.column_name,
		  c.data_type,
	  ...
	  `
        ...
        `
				...
				const relations: Relation[] = relationsResult.rows.map(
					(relation) => ({
						...
						relationType: this.determineRelationType(
							relation.is_unique_constraint,
							relation.is_primary_key_constraint,
						),
					}),
				)

				schemaInfo.push({ tableName, tableComment, columns, relations })
			}
